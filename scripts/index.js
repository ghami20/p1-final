...
document.getElementById("formularioCompra").addEventListener("submit", function (e) {
  ...
  const ciudad = datos.get("ciudad").trim();
  const tarjeta = datos.get("tarjeta").trim();
  const email = datos.get("email").trim();

  if (!nombre || !direccion || !ciudad || !tarjeta || !email) {
    mostrarToast("Por favor, completá todos los campos.", "error");
    return;
  }

  if (!/^\d{16}$/.test(tarjeta)) {
    mostrarToast("El número de tarjeta debe tener exactamente 16 dígitos numéricos.", "error");
    return;
  }
  ...
  mostrarToast(`Gracias ${nombre}, tu compra por $${total} fue realizada.`);
